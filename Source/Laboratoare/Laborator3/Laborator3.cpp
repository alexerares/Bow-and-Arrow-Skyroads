...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	new_y = new_y + sign * val * deltaTimeSeconds;
	modelMatrix *= Transform2D::Translate(x, new_y);
	...
	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(x_rot, y_rot);
	//TODO create animations by multiplying current transform matrix with matrices from Transform 2D

	u = u + val2 * deltaTimeSeconds;
	
	modelMatrix *= Transform2D::Rotate(u);

	if (u > 360)
		u = 0;

	modelMatrix *= Transform2D::Translate(x_rotm, y_rotm);
	...
	sclx = sclx + sign2 * val3 * deltaTimeSeconds;
	modelMatrix *= Transform2D::Scale(sclx, sclx);
	...
	modelMatrix *= Transform2D::Translate(x_bon, y_bon);
	z = z + sign * val4 * deltaTimeSeconds;

	modelMatrix *= Transform2D::Rotate(z);
	...
	modelMatrix *= Transform2D::Translate(-50, -50);
	modelMatrix *= Transform2D::Translate(x_bonm, y_bonm);
	//modelMatrix *= Transform2D::Translate(350, 250);
