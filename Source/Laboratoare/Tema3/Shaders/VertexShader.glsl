...
void main()
{
	...
	//gl_Position = MVP*vec4(v_position.x, y, v_position.z,1);
	...
	// TODO: compute light
	float Kc = 1,Kl = 0.11,Kq = 0.08;
	float d = distance(light_position,v_position);
	float intensitate = 1/(Kc+Kl*d+Kq*d*d);
	float light = ambient_light + intensitate * (diffuse_light + specular_light);
