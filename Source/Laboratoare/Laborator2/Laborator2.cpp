...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		vector<VertexFormat> vertices
		{
			...
			VertexFormat(glm::vec3(0, 0,  4), glm::vec3(1, 1, 1)),
			VertexFormat(glm::vec3(3.3, 0,  2), glm::vec3(1, 0, 1)),
			VertexFormat(glm::vec3(1.45, 3,  2), glm::vec3(1, 1, 0)),
		...
		vector<VertexFormat> vertices2
		{
			...
			VertexFormat(glm::vec3(5, 0,  3), glm::vec3(1, 0, 1)),
			VertexFormat(glm::vec3(5, 0,  5), glm::vec3(1, 0, 1)),
			VertexFormat(glm::vec3(3, 2,  3), glm::vec3(0, 4, 1)),
			VertexFormat(glm::vec3(3, 2,  5), glm::vec3(0, 4, 1)),
			VertexFormat(glm::vec3(5, 2,  3), glm::vec3(1, 0, 4)),
			VertexFormat(glm::vec3(5, 2,  5), glm::vec3(1, 0, 4)),
			// TODO: Complete the information for the cube
		};

		vector<VertexFormat> vertices3
		{
			VertexFormat(glm::vec3(-3, 0,  -5), glm::vec3(1, 0, 1)),
			VertexFormat(glm::vec3(-5, 0,  -5), glm::vec3(1, 0, 1)),
			VertexFormat(glm::vec3(-3, 2,  -5), glm::vec3(1, 0, 4)),
			VertexFormat(glm::vec3(-5, 2,  -5), glm::vec3(1, 0, 4)),
			// TODO: Complete the information for the cube
		};
		...
		vertices4.push_back(VertexFormat(glm::vec3(circlex, circley, 0), glm::vec3(1, 0, 1)));

		for(int i = 1; i <= nrtri; i++) {

			vertices4.push_back(VertexFormat(glm::vec3(circlex + (radius * cos(i * 6.28 / nrtri)), circley + (radius * sin(i * 6.28 / nrtri)), 0), glm::vec3(1, 0, 1)));
		...
		meshes["cubul"] = new Mesh("generated cubul");
		meshes["cubul"]->InitFromData(vertices2, indices2);

		meshes["patrat"] = new Mesh("generated patrat");
		meshes["patrat"]->InitFromData(vertices3, indices3);

		meshes["disc"] = new Mesh("generated patrat");
		meshes["disc"]->InitFromData(vertices4, indices4);

		// Create a new mesh from buffer data
		Mesh *cube = CreateMesh("cube3", vertices, indices);
		// Create a new mesh from buffer data
		Mesh* cubul = CreateMesh("cubu", vertices2, indices2);
		Mesh* patrat = CreateMesh("patrat", vertices3, indices3);

		Mesh* discul = CreateMesh("discul", vertices4, indices4);
	}
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["patrat"], shaders["VertexColor"], glm::vec3(-1.5f, 0.5f, 0), glm::vec3(0.5f));
	...
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(-1.5f, 1.5f, 0), glm::vec3(0.25f));

	RenderMesh(meshes["cubul"], shaders["VertexColor"], glm::vec3(-1.5f, 0.5f, 0), glm::vec3(0.5f));

	RenderMesh(meshes["disc"], shaders["VertexColor"], glm::vec3(-1.5f, 0.5f, 0), glm::vec3(0.5f));
